...
select
        poll_id, name, question,
        case when enabled_p = 't' then 1 else 0 end as enabled_p,
        case when active_p  = 't' then 1 else 0 end as active_p,
        case when acs_permission__permission_p(poll_id, :user_id, 'write')  = 't' then 1 else 0 end as edit_p,
        case when acs_permission__permission_p(poll_id, :user_id, 'delete') = 't' then 1 else 0 end as delete_p,
